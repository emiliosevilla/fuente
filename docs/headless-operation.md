...
Funes can run as a **continuous background worker** without Tkinter or PyWebView.
...
| Command | Use case |
|---------|----------|
| `funes --headless --vault /path/to/vault` | Continuous ingestion + graph refinement (Docker default) |
| `funes --flush --vault /path/to/vault` | One-shot deterministic pass; no background threads |
| `funes --vault /path/to/vault` | Desktop GUI (requires a display) |
...
Mount your Obsidian Vault at `/vault` (or any path passed to `--vault`). Funes
...
## State database (`.funes/state.db`)

ETL job progress is stored in `<vault>/.funes/state.db` (SQLite). The headless
worker resumes interrupted jobs on startup via `ApplicationLifecycle` and
`FolderMonitor`. Keep `.funes/` on the same volume as the Vault; deleting it
forces a full re-ingest of pending files.

Other `.funes/` contents:
...
`funes/config.py`). Non-loopback URLs (e.g. the `ollama` service hostname)
...
The Compose stack starts an empty Ollama service. Funes can connect to Ollama
...
```bash
docker exec -it funes_ollama_service ollama pull qwen2.5:7b
```

Repeat for any other model you configure via `custom_model_override` in
`.funes/config.json`. Model weights persist in the `ollama_storage` volume.
...
| | GUI (`funes --vault …`) | Headless (`funes --headless …`) |
|--|-------------------------|----------------------------------|
...
If you launch the default GUI command without a display, Funes exits immediately
