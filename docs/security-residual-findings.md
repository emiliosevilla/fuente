...
| SEC-002 | P2 | CSP / UI | parked | Static CSP/DOM safety regressions passed in `tests/test_html_safety_contract.py`; the required human visual console checkpoint has not been performed, so no full visual-resolution claim is made |
...
| SEC-006 | P2 | Tooling | resolved (not reproducible) | Both `pytest --collect-only -q` and `python3 -m pytest --collect-only -q` collected 571 tests on 2026-08-10; `python3 -m pytest` remains the canonical Unicode-path-safe command |
...
Subtask 9A residual evidence: the focused matrix passed **167 tests** with one external Chroma deprecation warning. Both collect-only commands collected **571 tests**. The two known global `RAMGovernor` failures remain outside this residual matrix and are not reclassified here.
...
The clean-tree checkpoint and release-gate results remain pending; this ledger does not claim either gate passed.
