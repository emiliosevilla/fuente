...
Estado activo inicial de la campaña: todas las fases estaban `NOT_RUN`. Estado actual: PR-00, PR-04 y PR-05 tienen `S PASS`, `R PASS` y están `COMPLETE`; PR-06 tiene `S PASS`, `R NOT_RUN` y está `PARTIAL`; PR-07 y las fases posteriores siguen `NOT_RUN`. El orden obligatorio es: PR-00, PR-04, PR-05, PR-06, PR-07, PR-01, PR-03, PR-08, PR-09, PR-10, PR-11, PR-02, PR-12. Cada fase ejecuta primero `S` sintética y sólo si pasa ejecuta `R` real.

`PR-10` repite su prueba sintética y no hereda automáticamente el bloqueo histórico por IDs duplicados. En el estado actual PR-00, PR-04 y PR-05 están `COMPLETE`; PR-06 está `PARTIAL` (`S PASS`, `R NOT_RUN`); PR-07+ están `NOT_RUN`.
