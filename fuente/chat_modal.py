"""
Fuente Chat Modal — Interfaz de chat nativa estilo Papiro.
...
try:
    from fuente.control_console import THEME, FONT_TYPEWRITER
except ImportError:
    THEME = {
        "bg_root": "#DCD4C7",
        "bg_card": "#EAE2D5",
...
class FuenteChatModal(tk.Toplevel):
    """Ventana Modal Nativa Papiro para 'Fuente el conversador'."""
    ...
    def __init__(
        ...
        *,
        ...
        self.title("Fuente el Conversador — Chat Inteligente Local")
    ...
    def _default_process_chat(
        ...
        policy = resolve_runtime_policy(self.config, budget=None)
        ram = RAMGovernor(
        ...
        with JobStore(vault.config.vault_path) as store:
            ...
            def eligible_hit(hit: Dict[str, Any]) -> bool:
                ...
                if relative_path.startswith("3_limpio/"):
                    try:
                        ...
                        return False
                    return True
                origins_data = metadata.get("origins_json", metadata.get("origins"))
                if origins_data is None:
                    return False
                try:
                    origins = parse_origins(
                        json.loads(origins_data)
                        if isinstance(origins_data, str)
                        else origins_data
                    )
                    ...
                    notes.require_eligible_origin_refs(origins, requires_origins=True)
                except (TypeError, ValueError, CanonicalEligibilityError):
                    return False
                return True
            ...
            if policy.vector_index_enabled:
                retrieval = RetrievalApplicationService(
                    ChromaStore(self.config.vault.chroma_dir),
                    runtime_policy=policy,
                    ram_governor=ram,
    ...
    def _setup_ui(self):
        ...
        tk.Label(
            status_bar,
            text="Fuente funciona con una IA 100% local, garantizando la privacidad",
            font=(FONT_TYPEWRITER, 8),
            fg=THEME["muted"],
            bg=THEME["bg_card"],
        ).pack(side="left")
    ...
    def _ping_url(self, url: str) -> bool:
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "FuenteConsole/2026"})
