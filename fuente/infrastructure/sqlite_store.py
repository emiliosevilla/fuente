...
Persists ETL job state at `<vault>/.fuente/state.db` so processing survives
...
class JobStore:
    ...
    One instance owns one `sqlite3.Connection` to `<vault>/.fuente/state.db`.
    ...
    def __init__(
        ...
        *,
        ...
        self.state_dir = self.vault_root / ".fuente"
    ...
    def migrate_note_vocabulary(
        ...
        *,
        ...
        """Commit one v3 catalog row without changing its semantic revision.

        Approval rows bound to the pre-migration Markdown bytes move to the
        corresponding post-migration hash in the same SQLite transaction.
        Reviewer, timestamp, invalidation state, aliases and route stay intact.
        """
        ...
        with self._immediate_transaction(note_id) as connection:
            ...
            if row is None:
                if catalog_existed:
                    raise IdentityCollisionError(
                        f"catalog row disappeared during v3 migration: {note_id!r}"
                    )
                connection.execute(
                    """
                    INSERT INTO note_catalog (
                ...
                if (
                    ...
                    raise IdentityCollisionError(
                        f"catalog identity changed during v3 migration: {note_id!r}"
                    )
                if str(row["content_hash"]) == post_content_hash:
                    if (
                        str(row["note_type"]) != note_type
                        or row["origin_kind"] != origin_kind
                    ):
                        raise IdentityCollisionError(
                            f"catalog vocabulary conflicts with v3 manifest: {note_id!r}"
                        )
                elif str(row["content_hash"]) == pre_content_hash:
                    connection.execute(
                        """
                        UPDATE note_approvals
                        SET content_hash = ?
                        WHERE note_id = ? AND revision = ? AND content_hash = ?
                        """,
                        (post_content_hash, note_id, revision, pre_content_hash),
                    )
                    changed = connection.execute(
                        """
                        UPDATE note_catalog
                        SET content_hash = ?, note_type = ?, origin_kind = ?, updated_at = ?
                        WHERE note_id = ? AND relative_path = ?
                    ...
                    if changed.rowcount != 1:
                        raise IdentityCollisionError(
                            f"catalog CAS failed during v3 migration: {note_id!r}"
                        )
                else:
                    raise IdentityCollisionError(
                        f"catalog content changed during v3 migration: {note_id!r}"
                    )
    ...
    def rollback_note_vocabulary(
        ...
        *,
        ...
        """Restore the exact pre-migration catalog vocabulary and approval hash."""
        with self._immediate_transaction(note_id) as connection:
            row = connection.execute(
                "SELECT * FROM note_catalog WHERE note_id = ?",
                (note_id,),
            ...
            if not catalog_existed:
                ...
                if (
                    str(row["relative_path"]) != relative_path
                    or int(row["revision"]) != revision
                    or str(row["content_hash"]) != post_content_hash
                ):
                    raise IdentityCollisionError(
                        f"catalog changed after v3 migration: {note_id!r}"
                    )
                dependent = connection.execute(
                    ...
                    SELECT
                        ...
                        (SELECT COUNT(*) FROM note_tombstones WHERE note_id = ?)
                    """,
                    (note_id, note_id, note_id),
                ).fetchone()[0]
                if dependent:
                    raise IdentityCollisionError(
                        f"catalog gained dependent state after v3 migration: {note_id!r}"
                    )
                connection.execute("DELETE FROM note_catalog WHERE note_id = ?", (note_id,))
                return

            if row is None:
                raise IdentityCollisionError(
                    f"catalog row disappeared after v3 migration: {note_id!r}"
                )
            if (
                str(row["relative_path"]) != relative_path
                or int(row["revision"]) != revision
                ...
                raise IdentityCollisionError(
                    f"catalog identity changed after v3 migration: {note_id!r}"
                )
            if str(row["content_hash"]) == pre_content_hash:
                if str(row["note_type"]) != note_type or row["origin_kind"] != origin_kind:
                    raise IdentityCollisionError(
                        f"catalog rollback vocabulary conflict: {note_id!r}"
                    )
                return
            if str(row["content_hash"]) != post_content_hash:
                raise IdentityCollisionError(
                    f"catalog content changed after v3 migration: {note_id!r}"
                )
            connection.execute(
                """
                UPDATE note_approvals
                ...
                WHERE note_id = ? AND revision = ? AND content_hash = ?
                """,
                (pre_content_hash, note_id, revision, post_content_hash),
            )
            changed = connection.execute(
                """
                UPDATE note_catalog
                ...
                (
                    ...
                    note_type,
                    origin_kind,
                    original_updated_at or _timestamp(),
                    note_id,
                    relative_path,
                    revision,
                    post_content_hash,
            ...
            if changed.rowcount != 1:
                raise IdentityCollisionError(
                    f"catalog rollback CAS failed: {note_id!r}"
                )
    ...
    def invalidate_note_approval(
        ...
        *,
        ...
        """Invalidate active approvals, bump revision, and mark derivatives."""
        marked_at = _timestamp()
        unique_derived_ids = sorted(set(derived_note_ids))
        with self._immediate_transaction(note_id) as connection:
            catalog = connection.execute(
                """
                SELECT catalog.*
            ...
            if catalog is None or str(catalog["content_hash"]) == new_content_hash:
                return 0
            ...
            if changed.rowcount != 1:
                raise sqlite3.IntegrityError("approval invalidation lost its catalog CAS")
            connection.executemany(
                """
                INSERT INTO derived_staleness (
                    origin_note_id, derived_note_id, marked_at
                ) VALUES (?, ?, ?)
                ON CONFLICT(origin_note_id, derived_note_id)
                DO UPDATE SET marked_at = excluded.marked_at
                """,
                [
                    (note_id, derived_note_id, marked_at)
                    for derived_note_id in unique_derived_ids
                ],
            )
            result = invalidated.rowcount
        return result

    def list_derived_staleness(self, origin_note_id: str) -> list[dict[str, Any]]:
        rows = self._connection.execute(
            """
            SELECT origin_note_id, derived_note_id, marked_at
            FROM derived_staleness
            WHERE origin_note_id = ?
            ORDER BY derived_note_id ASC
            """,
            (origin_note_id,),
        ).fetchall()
        return [dict(row) for row in rows]
    ...
    def relocate_note_cas(
        ...
        *,
        ...
        """Move a catalogued note without changing its semantic identity.

        A physical taxonomy move changes neither Markdown bytes nor revision.
        Keeping both values fixed preserves approvals and aliases; the exact
        current route, revision and hash form the CAS boundary.
        """
        try:
            cursor = self._connection.execute(
                """
                ...
                WHERE note_id = ? AND relative_path = ?
                  AND revision = ? AND content_hash = ?
                  AND NOT EXISTS (
                      SELECT 1 FROM note_tombstones
                      WHERE note_tombstones.note_id = note_catalog.note_id
                  )
                """,
                (
                    relative_path,
