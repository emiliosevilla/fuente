"""
Fuente Reader Modal — Visor y lector nativo de Notas Preparadas en Estética Papiro.
...
class FuenteReaderModal(tk.Toplevel):
    ...
    def __init__(
        ...
        backend: Optional["FuenteConsoleBackend"] = None,
    ):
        ...
        self.title("Fuente el Memorioso — Lector de Notas Preparadas")
    ...
    def _require_published_output(self) -> bool:
        if not self.current_document_id:
            return False
        try:
            self.backend.get_notes_service().require_published_output(
    ...
    def _copy_to_clipboard(self):
        ...
        if path is None or not path.exists():
            ...
            return
        if not self._require_published_output():
            return
