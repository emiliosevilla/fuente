...
def configure_anythingllm_integration(output_dir: Path) -> bool:
    ...
    try:
        ...
        env_lines = [
            ...
            "VECTOR_DB=lancedb",
            f"FUNES_OUTPUT_DIR={output_dir.resolve()}"
        ]
        ...
        if db_path.exists():
            try:
                ...
                cursor = conn.cursor()
                # Verificar o insertar workspace 'Habla con Funes'
                cursor.execute("SELECT id FROM workspaces WHERE slug = 'habla-con-funes'")
                row = cursor.fetchone()
                if not row:
                    cursor.execute(
                        "INSERT INTO workspaces (name, slug, createdAt, updatedAt) VALUES (?, ?, datetime('now'), datetime('now'))",
                        ("Habla con Funes", "habla-con-funes")
                    )
                    conn.commit()
                    logger.info("[+] Workspace 'Habla con Funes' registrado en la DB de AnythingLLM.")
                conn.close()
