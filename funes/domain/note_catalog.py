...
class NoteCatalog:
    ...
    def rebuild_from_markdown(self) -> ReconciliationReport:
        """Explicitly rebuild active catalog rows from valid v2 Markdown."""
        collisions: list[str] = []
        invalid: list[str] = []
        for path in sorted(self.vault_root.rglob("*.md")):
            if ".funes" in path.relative_to(self.vault_root).parts:
                continue
            relative_path = path.relative_to(self.vault_root).as_posix()
            try:
                ...
                continue
            note_id = metadata.get("note_id")
            if metadata.get("schema_version") != 2 or not isinstance(note_id, str):
                continue
