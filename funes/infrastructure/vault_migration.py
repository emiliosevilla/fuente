...
from funes.domain.documents import MarkdownDocument, content_hash_for_markdown
from funes.domain.errors import PathAuthorizationError
from funes.domain.frontmatter import (
    ALLOWED_STATUSES,
    FrontmatterError,
    _STATUS_MIGRATIONS,
    _split_frontmatter,
...
class VaultMigrator:
    ...
    def _refresh_moc_catalog(self) -> list[str]:
        """Regenerate MOC files without rewriting note bodies."""
        processed: list[str] = []
        for theme in self.vault.get_available_themes():
            ...
            for issue_name, issue_notes in sorted(notes_by_issue.items()):
                ...
                for note in issue_notes:
                    ...
                    note_path = output_dir / note.relative_path
                    try:
            ...
            loop._update_moc_index(all_notes, note_contents, orphans, issue_summaries)
