...
def select_llm_model(snapshot: MemorySnapshot) -> BudgetDecision:
    """Pick an LLM from the catalog and always attach an explainable reason."""
    if not snapshot.is_measured:
        eco = _ECO_MODEL
        return BudgetDecision(
            allowed=True,
    ...
    if not fit.allowed:
        # Walk down the catalog until a model fits usable headroom.
        chosen_index = next(
            (i for i, entry in enumerate(MODEL_CATALOG) if entry.id == chosen.id),
            0,
        )
        for candidate in MODEL_CATALOG[chosen_index::-1]:
...
def viable_models(snapshot: MemorySnapshot) -> List[Dict[str, Any]]:
    ...
    viable = [m.to_dict() for m in MODEL_CATALOG if m.min_ram_gb <= max_safe_ram]
    if not viable:
