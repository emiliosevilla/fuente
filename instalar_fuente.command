...
echo ""
echo "Creando acceso directo ejecutable Fuente.command en el Escritorio..."
if [ -f "./Fuente_macOS" ]; then
    chmod +x ./Fuente_macOS
fi
...
if ! ./venv/bin/python3 -c "from fuente.installer_contract import is_ollama_api_ready, start_ollama_service; import sys; sys.exit(0 if (is_ollama_api_ready() or start_ollama_service()) else 1)"; then
...
if [ -f "./venv/bin/python3" ]; then
    ./venv/bin/python3 -m fuente.installer_gui
else
    python3 -m fuente.installer_gui
fi
