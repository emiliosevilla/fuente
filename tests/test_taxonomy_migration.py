...
def test_plan_maps_typed_notes_and_never_moves(taxonomy_vault: Path) -> None:
    note = taxonomy_vault / "4_procesado" / "_Sin_Cuestion" / "meeting.md"
    ...
    assert manifest.entries[0].new_relative_path == "4_procesado/Fuentes/Reuniones/meeting.md"
    assert note.is_file()
    assert not (taxonomy_vault / "4_procesado" / "Fuentes").exists()


def test_apply_moves_and_rollback_restores(taxonomy_vault: Path) -> None:
    note = taxonomy_vault / "4_procesado" / "_Sin_Cuestion" / "concept.md"
    note.parent.mkdir(parents=True)
    ...
    moved = taxonomy_vault / "4_procesado" / "Conceptos" / "concept.md"
...
def test_plan_blocks_legacy_notes_until_identity_backfill(taxonomy_vault: Path) -> None:
    note = taxonomy_vault / "4_procesado" / "_Sin_Cuestion" / "legacy.md"
    note.parent.mkdir(parents=True)
    note.write_text("---\ntitle: Legacy\n---\n# Legacy\n", encoding="utf-8")
...
def test_apply_refuses_human_edit_after_plan(taxonomy_vault: Path) -> None:
    note = taxonomy_vault / "4_procesado" / "_Sin_Cuestion" / "topic.md"
    note.parent.mkdir(parents=True)
...
def _approved_origin(vault: Path) -> dict:
    note_id = str(uuid4())
    path = vault / "3_capturado" / f"origen-{note_id}.md"
...
def _eligible_summary(vault: Path, *, relative: str = "4_procesado/Fuentes/a.md") -> Path:
...
def test_sumarios_dry_run_moves_only_v3_summaries_and_keeps_clean_intact(taxonomy_vault: Path) -> None:
    ...
    clean = next((taxonomy_vault / "3_capturado").glob("origen-*.md"))

    manifest = plan_sumarios_migration(taxonomy_vault)

    assert manifest.status == "dry_run"
    assert len(manifest.entries) == 1
    assert manifest.entries[0].old_relative_path == "4_procesado/Fuentes/a.md"
    assert manifest.entries[0].new_relative_path == "4_procesado/Sumarios/Reuniones/a.md"
    assert summary.is_file()
    assert clean.is_file()
    assert not (taxonomy_vault / "4_procesado/Sumarios/Reuniones/a.md").exists()
...
def test_sumarios_route_validation_keeps_theme_prefix_and_excludes_clean(taxonomy_vault: Path) -> None:
    ...
    assert not migrator._is_exact_sumarios_route(
        "TemaA/4_procesado/Fuentes/a.md",
        "TemaB/4_procesado/Sumarios/Reuniones/a.md",
    )
    assert not migrator._is_exact_sumarios_route(
        "3_capturado/Fuentes/a.md",
        "3_capturado/Sumarios/Reuniones/a.md",
    )


def test_sumarios_blocks_legacy_non_summary_incomplete_unapproved_collision_and_clean(taxonomy_vault: Path) -> None:
    legacy = taxonomy_vault / "4_procesado/Fuentes/legacy.md"
    legacy.parent.mkdir(parents=True)
    legacy.write_text(_note(str(uuid4()), "source", "meeting"), encoding="utf-8")
    non_summary = taxonomy_vault / "4_procesado/Fuentes/concept.md"
    non_summary.write_text(_v3_markdown(str(uuid4()), "concept"), encoding="utf-8")
    incomplete = taxonomy_vault / "4_procesado/Fuentes/incomplete.md"
    incomplete.write_text("---\nschema_version: 3\norigins: []\n---\n# rota\n", encoding="utf-8")
    unapproved_id = str(uuid4())
    unapproved = taxonomy_vault / "4_procesado/Fuentes/unapproved.md"
    unapproved.write_text(
        _v3_markdown(
            unapproved_id,
            "summary",
            origin_kind="meeting",
            origins=[{"note_id": str(uuid4()), "revision": 1, "content_hash": "0" * 64, "path": "3_capturado/no.md"}],
        ),
        encoding="utf-8",
    )
    with JobStore(taxonomy_vault) as store:
        store.register_note(
            ...
            relative_path="4_procesado/Fuentes/unapproved.md",
    ...
    _eligible_summary(taxonomy_vault, relative="4_procesado/Fuentes/a.md")
    _eligible_summary(taxonomy_vault, relative="4_procesado/_Sin_Cuestion/a.md")
    clean = taxonomy_vault / "3_capturado" / "untouched.md"
    ...
    assert all("3_capturado" not in finding.relative_path for finding in manifest.findings)
    assert clean.is_file()
...
def test_sumarios_apply_requires_human_manifest_approval_and_preserves_catalog(taxonomy_vault: Path) -> None:
    ...
    destination = taxonomy_vault / "4_procesado/Sumarios/Reuniones/a.md"
    ...
    with JobStore(taxonomy_vault) as store:
        ...
        assert row["relative_path"] == "4_procesado/Sumarios/Reuniones/a.md"
...
def test_sumarios_apply_rejects_manifest_changed_after_approval(taxonomy_vault: Path) -> None:
    ...
    payload["entries"][0]["new_relative_path"] = "4_procesado/Sumarios/Correos/a.md"
...
def test_sumarios_apply_rejects_catalog_destination_collision_before_rename(taxonomy_vault: Path) -> None:
    source = _eligible_summary(taxonomy_vault)
    manifest_path = _approved_sumarios_manifest(taxonomy_vault)
    destination = "4_procesado/Sumarios/Reuniones/a.md"
    with JobStore(taxonomy_vault) as store:
        store.register_note(
            note_id=str(uuid4()),
...
def test_sumarios_rollback_refuses_a_human_edited_file(taxonomy_vault: Path) -> None:
    ...
    assert not (taxonomy_vault / "4_procesado/Fuentes/a.md").exists()


def test_sumarios_rewrites_only_an_exact_wikilink_to_a_moved_route(taxonomy_vault: Path) -> None:
    _eligible_summary(taxonomy_vault)
    holder = taxonomy_vault / "4_procesado/Conceptos/holder.md"
    holder.parent.mkdir(parents=True)
    holder.write_text(
        _v3_markdown(
            str(uuid4()),
            "concept",
        body="# Holder\n\n[[4_procesado/Fuentes/a]]\n\n4_procesado/Fuentes/a\n\n```md\n[[4_procesado/Fuentes/a]]\n```\n",
        ),
        encoding="utf-8",
    )
    manifest_path = _approved_sumarios_manifest(taxonomy_vault)
    ...
    assert "[[4_procesado/Sumarios/Reuniones/a]]" in body
    assert "\n\n4_procesado/Fuentes/a\n" in body
    assert "```md\n[[4_procesado/Fuentes/a]]\n```" in body
...
def test_sumarios_rollback_restores_wikilinks_when_unchanged(taxonomy_vault: Path) -> None:
    ...
    apply_sumarios_migration(manifest_path)
    assert "[[4_procesado/Sumarios/Reuniones/a]]" in holder.read_text(encoding="utf-8")
...
def test_sumarios_rollback_keeps_link_when_target_move_was_not_rolled_back(taxonomy_vault: Path) -> None:
    _eligible_summary(taxonomy_vault)
    holder = taxonomy_vault / "4_procesado/Sumarios/Reuniones/holder.md"
    holder.parent.mkdir(parents=True)
    holder.write_text(
        _v3_markdown(
            str(uuid4()),
